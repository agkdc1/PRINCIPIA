...
/-!
# Neumann natural BC Laplacian (NBC_03)

Target T20c_13 Courant+Hilbert *MMP* I (1924).  Carrier for HVT `NBC_03`.
...
namespace NeumannLaplacian

structure NeumannBCData where
  neumann : Prop

/-- Upstream-narrow axiom for NBC_03 HVT.
    Citation: Courant-Hilbert *Methoden der mathematischen Physik* I (1924).
    Promoted from B3 structural witness to explicit axiom per c4 audit response,
    aligned with Step 5 defer classification + hard rule "Direction-over-count —
    upstream-narrow axioms OK". Proof deferred to Tier-3/4 drilldown. -/
axiom nbc_03_witness (D : NeumannBCData) : ∃ x : Prop, x = D.neumann

end NeumannLaplacian
