...
/-!
# Chapter B branch-log contour lemmas

This file records the part of the argument-principle story that is honestly
available on the current Mathlib pin:

- antiderivative theorems for the Chapter A `C¹` / piecewise-`C¹` contour API;
- vanishing of `∮ f'/f` on closed contours when `f` admits a global `Complex.log`
  branch along the contour image;
- normalized sector-angle identities for the Chapter A shrinking-indent arc
  primitive;
- a rectangle-boundary specialization in the same branch-cut-free setting.

The full meromorphic argument principle on piecewise-`C¹` Jordan boundaries is
still blocked in this environment by the missing winding-number / residue /
Jordan-curve substrate. That honest wall is documented separately in
`VALENCE_CHAPTER_B_ARGUMENT_PRINCIPLE_BREACH_REPORT.md`.
-/
...
namespace MathlibExpansion.Roots.Valence.ArgumentPrinciple

open MathlibExpansion.Roots.Valence.CompositeContour
...
theorem integral_eq_sub_of_hasDerivAt (γ : C1Path z₀ z₁) {F H : ℂ → ℂ}
  ...
  have hderivAt : ∀ t : ℝ, HasDerivAt G (γ.deriv t * F (γ t)) t := by
    intro t
    simpa [G, Function.comp_apply, smul_eq_mul] using (hH (γ t)).scomp t (γ.hasDerivAt_toFun t)
...
theorem integral_logDeriv_eq_log_sub (γ : C1Path z₀ z₁) {f f' : ℂ → ℂ}
  ...
  · intro z
    simpa [Function.comp_apply, logDeriv, Pi.div_apply, (hf z).deriv] using (hf z).clog (hslit z)
...
theorem normalized_integral_sub_center_inv_int_arc {c : ℂ} {R θ₀ θ₁ : ℝ} {n : ℤ} (hR : R ≠ 0) :
    ((2 * (Real.pi : ℂ) * Complex.I)⁻¹) *
        (MathlibExpansion.Roots.Valence.CompositeContour.C1Path.arc c R θ₀ θ₁).integral
          (fun z => (z - c)⁻¹ * (n : ℂ)) =
      ((((θ₁ - θ₀) / (2 * Real.pi) : ℝ) : ℂ) * (n : ℂ)) := by
  rw [MathlibExpansion.Roots.Valence.CompositeContour.C1Path.integral_sub_center_inv_const_arc
    (res := (n : ℂ)) hR]
...
theorem tendsto_normalized_integral_indent_arc_int {c : ℂ} {ρ θ₀ θ₁ : ℝ} {n : ℤ} (hρ : 0 < ρ)
    ...
    Tendsto
      (fun ε : ℝ =>
        ((2 * (Real.pi : ℂ) * Complex.I)⁻¹) *
          (MathlibExpansion.Roots.Valence.CompositeContour.C1Path.arc c ε θ₀ θ₁).integral
            (fun z => (z - c)⁻¹ * (n : ℂ) + g z))
  ...
  have h :=
    MathlibExpansion.Roots.Valence.CompositeContour.C1Path.tendsto_integral_indent_arc
...
theorem normalized_full_turn_integral_sub_center_inv_int_arc
    ...
    ((2 * (Real.pi : ℂ) * Complex.I)⁻¹) *
        (MathlibExpansion.Roots.Valence.CompositeContour.C1Path.arc c R θ (θ + 2 * Real.pi)).integral
          (fun z => (z - c)⁻¹ * (n : ℂ)) =
      (n : ℂ) := by
...
theorem integral_eq_sub_of_hasDerivAt (γ : PiecewiseC1 z₀ z₁) {F H : ℂ → ℂ}
  ...
  | ofC1 γ =>
      simpa using
        MathlibExpansion.Roots.Valence.ArgumentPrinciple.C1Path.integral_eq_sub_of_hasDerivAt
          (γ := γ) hH hcontF
  | trans γ₁ γ₂ ih₁ ih₂ =>
      simp [ih₁, ih₂]
  | symm γ ih =>
...
/-- The positively oriented axis-aligned rectangle boundary with opposite corners
`z` and `w`, viewed as a closed piecewise-`C¹` contour. -/
  def rectangleBoundary (z w : ℂ) : PiecewiseC1 z z :=
  let zRight : ℂ := ⟨w.re, z.im⟩
  let zTop : ℂ := ⟨z.re, w.im⟩
  .trans (MathlibExpansion.Roots.Valence.CompositeContour.PiecewiseC1.line z zRight)
    (.trans (MathlibExpansion.Roots.Valence.CompositeContour.PiecewiseC1.line zRight w)
      (.trans (MathlibExpansion.Roots.Valence.CompositeContour.PiecewiseC1.line w zTop)
        (MathlibExpansion.Roots.Valence.CompositeContour.PiecewiseC1.line zTop z)))

@[simp]
theorem integral_logDeriv_rectangleBoundary_eq_zero (z w : ℂ) {f f' : ℂ → ℂ}
    (hf : ∀ u, HasDerivAt f (f' u) u) (hf' : Continuous f')
    (hslit : ∀ u, f u ∈ Complex.slitPlane) :
    (rectangleBoundary z w).integral (logDeriv f) = 0 :=
  integral_logDeriv_eq_zero_of_closed (γ := rectangleBoundary z w) hf hf' hslit

end PiecewiseC1

end MathlibExpansion.Roots.Valence.ArgumentPrinciple
