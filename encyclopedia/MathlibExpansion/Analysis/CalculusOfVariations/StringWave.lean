...
/-!
# String/beam wave equation from action (CHM-05)

Target T20c_13 Courant+Hilbert *MMP* I (1924).  Carrier for HVT `CHM_05`.
...
namespace StringWave

structure StringWaveData where
  wave : Prop

/-- Upstream-narrow axiom for CHM_05 HVT.
    Citation: Courant-Hilbert *Methoden der mathematischen Physik* I (1924).
    Promoted from B3 structural witness to explicit axiom per c4 audit response,
    aligned with Step 5 defer classification + hard rule "Direction-over-count —
    upstream-narrow axioms OK". Proof deferred to Tier-3/4 drilldown. -/
axiom chm_05_witness (D : StringWaveData) : ∃ x : Prop, x = D.wave
